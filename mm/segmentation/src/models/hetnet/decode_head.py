...
class HetNetHead(BaseDecodeHead):
    def __init__(self,
        ...
        self.head = nn.ModuleList([
                ...
                conv3x3(64, num_classes, bias=True),
                conv3x3(64, num_classes, bias=True),
                conv3x3(64, num_classes, bias=True),
                conv3x3(64, num_classes, bias=True),
                conv3x3(64, num_classes, bias=True),
    ...
    def loss_by_feat(self, seg_logits: Tuple[Tensor],
        ...
        out1, out_edge1, out2,  out3, out4, out5 = seg_logits 
        
        loss['loss1'] = structure_loss(out1, seg_label, self.loss_decode[0])
        loss['loss_edge'] = bce_loss(out_edge1, bd_label)
        loss['loss2'] = structure_loss(out2, seg_label, self.loss_decode[0])
        loss['loss3'] = structure_loss(out3, seg_label, self.loss_decode[0])
        loss['loss4'] = structure_loss(out4, seg_label, self.loss_decode[0])
        loss['loss5'] = structure_loss(out5, seg_label, self.loss_decode[0])
        
        loss['loss'] = loss['loss1'] + loss['loss_edge'] + loss['loss2']/2 + loss['loss3']/4 + loss['loss4']/8 + loss['loss5']/16
        
        return loss

    def initialize(self):
        # self.load_state_dict(torch.load('/HDD/weights/hetnet/PMD-model-best/model-best'))
        weight_init(self)    
...
def structure_loss(pred, mask, ce):
    ...
    ce_loss  = ce(pred, mask.squeeze(1).long())
    wbce  = (weit*ce_loss).sum(dim=(2, 3))/weit.sum(dim=(2, 3))
