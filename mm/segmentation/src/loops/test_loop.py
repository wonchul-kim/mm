...
def merge_tta_batch(batch_output, out_channels, decode_head_threshold=None):
...
def make_tta_batch(augs, new_data_batch, batch_input, batch_data_sample):
...
def recover_tta_output(augs, batch_output):
    idx = 0
    for key, val in augs.items():
        if key == 'HorizontalFlip' and val:
...
class TestLoopV2(TestLoop):
    ...
    def run_tta_batch(self, data_batch):               
        ...
        for batch_input, batch_data_sample in zip(batch_inputs, batch_data_samples):
            make_tta_batch(augs, new_data_batch, batch_input, batch_data_sample)
            batch_output = self.runner.model.test_step(new_data_batch)
            recover_tta_output(augs, batch_output)
            batch_outputs.append(merge_tta_batch(batch_output, self.runner.model.out_channels, self.runner.model.decode_head.threshold
