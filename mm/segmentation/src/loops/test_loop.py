...
class TTAModel(torch.nn.Module):
    ...
    def make_tta_batch(self, augs, new_data_batch, batch_input, batch_data_sample):
    ...
    def merge_tta_batch(self, batch_output, out_channels, decode_head_threshold=None):
        ...
        if out_channels == 1:
            seg_pred = (logits > decode_head_threshold).to(logits).squeeze(1)
        else:
    ...
    def recover_tta_output(self, augs, batch_output):
        idx = 0
        for key, val in augs.items():
            if key == 'HorizontalFlip' and val:
...
class TestLoopV2(TestLoop):
    ...
    def run_tta_batch(self, data_batch):               
        ...
        batch_outputs = []
        self.runner.model = TTAModel(self.runner.model)
        for batch_input, batch_data_sample in zip(batch_inputs, batch_data_samples):
            self.runner.model.make_tta_batch(augs, new_data_batch, batch_input, batch_data_sample)
            batch_output = self.runner.model.test_step(new_data_batch)
            self.runner.model.recover_tta_output(augs, batch_output)
            batch_outputs.append(self.runner.model.merge_tta_batch(batch_output, self.runner.model.out_channels, self.runner.model.decode_head.threshold
                                                 if hasattr(self.runner.model, 'decode_head') and hasattr(self.runner.model.decode_head, 'threshold') else None))
